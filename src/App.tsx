...
export function App() {
  ...
  const handleSmartRename = async () => {
    ...
    try {
      const suggestedName = await analyzeDocumentForRenaming(images[0].dataUrl, currentLang);
      if (suggestedName) {
        setPdfSettings((prev) => ({ ...prev, fileName: suggestedName }));
        showToast(
          currentLang === 'ar'
            ? `تم تعيين الاسم المقترح: ${suggestedName}`
            : `Suggested name applied: ${suggestedName}`,
          'success'
        );
      }
    } catch (err) {
